...
> 定位：个人重构与扩展项目，参考原项目并经作者许可。聚焦“创作者可靠发布到消费者互动”的短视频闭环；不将商城、直播、订单等存量 CRUD 作为简历亮点。

## 一句话概述

面向创作者与内容消费者的视频平台后端，围绕大文件直传、异步转码发布、关注流分发、互动一致性与可观测性重构核心链路；以轻量 AI 创作助手提供权限受控的处理状态查询和失败诊断。
...
- 补齐公开/关注双 Feed、点赞、收藏、评论和关注关系：采用游标分页避免深分页，互关不耦合聊天域；点赞/收藏使用 Redis 原子状态切换与事件 ID 消费幂等，评论通过 Outbox 异步聚合计数，兼顾交互响应与重复投递保护。
...
| 消费重复如何处理？ | 处理任务使用合法状态迁移，点赞/收藏事件以 `eventId` 写入幂等表，评论事件以 `comment_id` 去重；关注流 Inbox 用 `(recipient_id, video_id)` 唯一键，恢复记录用 `(message_digest, recovery_attempt)` 区分同一次崩溃补偿与新恢复尝试。 |
...
- 本机前端以 Vue 3 覆盖公开/关注流、点赞/收藏/评论、JWT 登录、创作者工作台、上传状态与创作助手；浏览器已走查真实公开 Feed、游标加载和互动入口，隔离账号已完成互动接口验收。不以页面数量作为项目亮点。
