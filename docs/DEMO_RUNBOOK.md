...
本文只覆盖简历主线：可靠视频异步处理、受限 AI 创作工具与可观测性。执行前请确认仅在本机开发环境操作；脚本会产生本地测试数据，但不输出密码或模型密钥。

## 0. 演示边界

- 演示的核心不是页面数量，而是“上传成功、失败可诊断、下游故障可隔离并可审计恢复”。
...
1. 业务难点：大文件不经 Java 网关，上传成功不等于可发布。
2. 一致性方案：业务状态与 Outbox 同事务，Consumer 侧以状态机和唯一键幂等。
3. 故障边界：处理回写有租约恢复；关注流下游故障有 Broker 延迟重试与最终 DLQ。
4. 恢复治理：DLQ 恢复需审计和补偿 Outbox，不能直接重放旧消息。
5. 证据：核心回归、E2E 成功/失败脚本、Prometheus/Grafana、固定环境延迟基线。
